...
1. `strain_perturbation` -- called from `engines/gradient/run.py`'s `run_model` when
   ...
   at a point that later moves), so plain jax autodiff yields d(loss)/d(eps) at that
   point -- the strain-gradient sensitivity -- with no need for a hand-derived adjoint.
...
2. `StrainDistortionSolver` -- a `GradientSolver` with params={'distortion'} that
   ...
   (ported near-verbatim from distort.py, CPU/SciPy -- no JAX equivalent for the
...
def solve_distortion(
    ...
    """Convert a per-position strain *update* (already scaled to a step size and sign-flipped
    to a descent direction -- NOT the raw loss gradient) into object and position updates.
    ...
    CPU/SciPy only -- no JAX equivalent for the scattered/gridded interpolation steps.
    """
...
class StrainDistortionSolverProps(Dataclass):
    ...
    """Fraction of the raw strain gradient to convert into a displacement-producing update."""
